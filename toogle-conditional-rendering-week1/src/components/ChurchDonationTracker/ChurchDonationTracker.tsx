...
export default function ChurchDonationTracker() {
  ...
  const filteredDonations = donations.filter((person) => {
    return person.donor
      .toLocaleLowerCase()
      .includes(searchQuery.searchDonor.toLocaleLowerCase());
  });
          ...
          <tbody>
            ...
            {filteredDonations.length > 0 ? (
              filteredDonations.map((item) => (
                ...
                >
                  <td className="p-5 font-bold text-slate-700 underline decoration-indigo-200">
                    {item.donor}
                  </td>
                  ...
                  <td className="p-5 text-right font-black text-indigo-600">
                    ₱ {privacyMode ? "****" : item.amount.toLocaleString()}
                  </td>
